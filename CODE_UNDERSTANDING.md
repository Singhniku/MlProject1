...
- `POST /api/resume/rewrite` ↔ Gemini `generateContent` API (model `gemini-2.0-flash`, override via `GEMINI_MODEL` env var; `call_gemini()`). Reads `GEMINI_API_KEY` from env or `config/.env` (via `load_env_file()`, gitignored, KEY=VALUE lines, `os.environ.setdefault` so real env wins — `ENV_FILE` constant, loaded before `GEMINI_MODEL`/`GEMINI_URL` are read). No key → 400 `NO_API_KEY` message pointing to `./setup`. HTTPError/URLError/malformed-JSON/empty-candidates all caught and returned as clean `{error}` JSON (never a raw traceback). System prompt in `REWRITE_SYSTEM_PROMPT` enforces verb→tech→scope→outcome bullets and forbids inventing facts.
...
Bash script (`chmod +x`). Checks python3 present → prompts once for `GEMINI_API_KEY` if `config/.env` has none and writes it there (chmod 600, gitignored — key never reaches the browser or the bundled artifact) → `python3 build.py` → frees port 8500 if in use (`lsof`) → `python3 server.py &` → opens the browser. Ctrl+C (trap) kills the server.
...
## Changelog
- 2026-07-17: moved the API key file from repo-root `.env` to `config/.env` (ENV_FILE constant in server.py); switched the Resume Modifier's AI rewrite from Anthropic to the Gemini API (`call_gemini()`, model `gemini-2.0-flash`, `GEMINI_API_KEY`). Verified end-to-end via the running preview: no-key path shows the correct error in the UI; a syntactically-valid fake key produced a genuine `API_KEY_INVALID` response from googleapis.com, confirming the request/endpoint/headers are correct — success path needs a real user key. ATS scoring re-verified unaffected (51/100 on a test resume).
